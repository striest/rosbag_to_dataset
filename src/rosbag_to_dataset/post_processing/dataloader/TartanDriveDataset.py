...
class DatasetBase(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for datatype, datalen in zip(self.datatypelist, self.modalitylenlist): 
            ...
            if datatype == 'img0' or datatype == 'img1' or datatype == 'imgc':
                imglist = self.load_image(datafilelist,datatype)
                if imglist is None:
                    print("!!!READ IMG ERROR {}, {}, {}".format(idx, trajstr, framestrlist, datafilelist))
                sample[datatype] = torch.from_numpy(np.stack(imglist))
            elif datatype == 'disp0' or datatype == 'heightmap' or datatype=='rgbmap':
                datalist = self.load_numpy(datafilelist,datatype)
                sample[datatype] = torch.from_numpy(np.stack(datalist))
            elif datatype == 'odom':
    ...
    def load_image(self, fns, datatype):
        imglist = []
        for fn in fns: 
            ...
            for modality in self.config["modalities"].keys():
                if self.remap_obs[datatype] == self.config["modalities"][modality]["topic"]:
                    if "output_resolution" in self.config["modalities"][modality].keys():
                        ...
                        img = cv2.resize(img, dsize=(output_resolution[0], output_resolution[1]), interpolation=cv2.INTER_AREA)
                    break
            imglist.append(img)
            assert img is not None, "Error loading image {}".format(fn)
        return imglist

    def load_numpy(self, fns, datatype):
        displist = []
        for fn in fns:
