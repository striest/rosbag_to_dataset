...
if __name__ == '__main__':
	...
	model_fp = os.path.join(args.experiment_fp,"_latest")
	latent_model = torch.load(os.path.join(model_fp,"model.cpt"))
	...
	summary_fp = os.path.join(model_fp,"_summary")
	with open(summary_fp,"r") as file:
		trainer.current_epoch = int(file.readline().split(" ")[-1])
	# if True:
	experiment.run()
