...
class PWCDCNet(nn.Module):
    ...
    def get_loss_old(self, output, target, criterion, small_scale=False):
        ...
        if self.training:
            ...
            loss1 = self.calc_loss(output[0], target4, criterion, mask=None, unc=output[5], lamb=1.0) #criterion(output[0], target4)
            loss2 = self.calc_loss(output[1], target8, criterion, mask=None, unc=output[6], lamb=1.0) #criterion(output[1], target8)
            loss3 = self.calc_loss(output[2], target16, criterion, mask=None, unc=output[7], lamb=1.0) #criterion(output[2], target16)
            loss4 = self.calc_loss(output[3], target32, criterion, mask=None, unc=output[8], lamb=1.0) #criterion(output[3], target32)
            loss5 = self.calc_loss(output[4], target64, criterion, mask=None, unc=output[9], lamb=1.0) #criterion(output[4], target64)
            ...
            if small_scale:
                ...
                if output[5] is not None:
                    ...
                    unc4 = None
            loss = self.calc_loss(output4, target, criterion, mask=None, unc=unc4, lamb=1.0) #criterion(output4, target)
        return loss
    ...
    def get_loss(self, output, target, criterion, mask=None, small_scale=False, turnoff_unc=False):
        '''
        return flow loss
        small_scale: True - the target and mask are of the same size with output
                     False - the target and mask are of 4 time size of the output
        ...
        if self.training: # PWCNet + training
            ...
            for k in range(5):
                if turnoff_unc:
                    ...
                    unc = output[5+k]
                losslist[k] = self.calc_loss(output[k], targetlist[k], criterion, mask=masklist[k], unc=unc) #criterion(output[0][mask4], target4[mask4])
            ...
            if turnoff_unc:
                unc4 = None
            loss = self.calc_loss(output4, target, criterion, mask=valid_mask, unc=unc4) #criterion(output4[valid_mask], target[valid_mask])
        return loss
